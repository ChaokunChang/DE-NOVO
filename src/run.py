...
def compressed_graph_mining(graph, discription,mode=3):
    k = discription['k']
...
class DBG():
    ...
    def get_answers(self,mode='simple',graph_path=None,table_path=None):
        ...
        for i in range(ans_num):
            node = output[i]
            if len(node) > 9000:
                combined_output.append(node)
                continue
            max_overlap = 0
            for j in range(i+1,ans_num):
                if len(output[j]) > 9000:
                    continue
                match_len = kmp(output[i],output[j])
                if match_len >= self.k/2:
                    combined_output.append(output[i]+output[j][match_len:])
                match_len = kmp(output[j],output[i])
                if match_len >= self.k/2:
                    combined_output.append(output[j]+output[i][match_len:])
